...
import { fetchYoutubeTranscription, fetchYoutubeCaptions, fetchYoutubeApiCaptions, analyzeTranscription } from "@/services/youtubeService";
...
const YouTubeAnalysis = () => {
  ...
  const [useApiCaptions, setUseApiCaptions] = useState<boolean>(false);
  ...
  const fetchTranscription = async () => {
    ...
    try {
      ...
      console.log(`Using ${useApiCaptions ? 'API captions' : useFallbackMethod ? 'alternative' : 'standard'} method`);
      
      let transcriptionText = "";
      
      if (useApiCaptions) {
        // Use direct API captions endpoint
        try {
          console.log("Using direct API captions method");
        ...
        try {
          ...
          try {
            ...
            console.error("Fallback method also failed, trying API captions:", fallbackError);
            setUseApiCaptions(true);
            toast.info("Trying API captions method...");
  ...
  const toggleTranscriptionMethod = () => {
    if (useApiCaptions) {
      setUseApiCaptions(false);
      setUseFallbackMethod(false);
      toast.info("Using standard transcription method");
    } else if (useFallbackMethod) {
      setUseApiCaptions(true);
      setUseFallbackMethod(false);
      toast.info("Using API captions method");
    } else {
      setUseFallbackMethod(true);
      setUseApiCaptions(false);
      toast.info("Using alternative transcription method");
    }
  };
